# Agent guide - unicode-ident-kotlin

This file is the quick-reference operating contract for unicode-ident-kotlin. The longer
...
unicode-ident-kotlin is a Kotlin Multiplatform port of
[dtolnay/unicode-ident](https://github.com/dtolnay/unicode-ident), which exposes
`is_xid_start(c)` and `is_xid_continue(c)` predicates that match Unicode
Standard Annex #31 identifier characters. Upstream Rust is the behavioral
oracle while the repo is still in parity mode. Never edit `tmp/` or any fetched
upstream source to make the port easier.
