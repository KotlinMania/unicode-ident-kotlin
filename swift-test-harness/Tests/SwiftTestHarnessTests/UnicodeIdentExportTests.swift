...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `UnicodeIdent.swiftmodule/`
...
//      `import UnicodeIdent` above would fail at compile time.
//
//   2. The static archive `libUnicodeIdent.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "UnicodeIdent" }` and
//      `flattenPackage = "io.github.kotlinmania.unicodeident"` configuration in
...
//      via the `UnicodeIdentLibrary` product.
//
...
final class UnicodeIdentExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "UnicodeIdent swift module imported cleanly")
    }
}
